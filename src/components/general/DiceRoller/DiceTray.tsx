...
export const DiceTray = (props: DiceTrayProps) => {
    ...
    useEffect(() => {
        ...
        const initUserTheme = async () => {
            ...
            if (!hopeTheme) {
                const themeId = room?.diceUser?.find((user) => user.playerId === playerContext.id)?.hopeTheme;
                if (themeId && !isNull(themes) && themes.map((t) => t.id).includes(themeId)) {
                    const newTheme = (await rollerApi?.theme.get(themeId))?.data;
                    if (newTheme) {
                        setHopeTheme(newTheme);
                    }
                }
            }
            if (!fearTheme) {
                const themeId = room?.diceUser?.find((user) => user.playerId === playerContext.id)?.fearTheme;
                if (themeId && !isNull(themes) && themes.map((t) => t.id).includes(themeId)) {
                    const newTheme = (await rollerApi?.theme.get(themeId))?.data;
                    if (newTheme) {
                        setFearTheme(newTheme);
                    }
                }
            }
        };
    ...
    }, [rollerApi, diceThemeQuery.isSuccess, diceUser?.diceTheme, diceUser?.hopeTheme, diceUser?.fearTheme, themes]);
