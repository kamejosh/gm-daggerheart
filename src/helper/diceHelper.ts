...
export const updateRoomMetadataDiceUser = async (
    ...
    if (index >= 0) {
        ...
        diceUser.splice(index, 1, {
            ...
            hopeTheme: options?.hopeTheme ?? user.hopeTheme ?? "dddice-bees",
            fearTheme: options?.fearTheme ?? user.fearTheme ?? "dddice-bees",
        ...
        diceUser.push({
            ...
            hopeTheme: options?.hopeTheme ?? "dddice-bees",
            fearTheme: options?.fearTheme ?? "dddice-bees",
...
export const getApiKey = async (room: RoomMetadata | null) => {
    ...
    if (!apiKey) {
        apiKey = (await new ThreeDDiceAPI(undefined, "GM's Daggerheart").user.guest()).data;
        updateKey = true;
    }
        ...
        try {
            ...
            if (e.response.status === 404) {
                ...
                if (room && newDiceRoom) {
                    ...
                    console.warn("GM's Daggerheart - unable to connect to dddice room");
                }
            }
        }
...
export const dddiceApiLogin = async (room: RoomMetadata | null, user?: IUser) => {
    const rollerApi = new ThreeDDiceAPI(await getApiKey(room), "GM's Daggerheart");
...
export const diceToRoll = (diceString: string, theme: string) => {
    ...
    if (diceString.includes("d")) {
        try {
            const parsed = parseRollEquation(diceString, theme || "dddice-bees");
            dice = parsed.dice;
            ...
            if (split.length === 2) {
                ...
                for (let i = 0; i < amount; i++) {
                    dice.splice(0, 0, { type: `d${parseInt(die)}`, theme: theme || "dddice-bees" });
                }
            }
        }
    } else if (diceString.startsWith("+") || diceString.startsWith("-")) {
        dice.push({ type: "d20", theme: theme || "dddice-bees" });
        dice.push({ type: "mod", theme: theme || "dddice-bees", value: parseInt(diceString) });
    } else {
